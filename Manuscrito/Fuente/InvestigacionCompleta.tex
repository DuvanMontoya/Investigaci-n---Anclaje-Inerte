...
\hypersetup{
  ...
  citecolor=black,
  urlcolor=black,
  pdftitle={De la esfera x2+y2+z2=R2 al anclaje inerte: densidades p-adicas explicitas para R2-c2},
...
% Título (RECOMENDADO)
% ---------------------------------------------------------------
\title{\textbf{De la esfera \(x^2+y^2+z^2=R^2\) al anclaje inerte:\\
densidades \(p\)-ádicas explícitas para la familia \(R^2-c^2\)}\\[2pt]
...
% ================================================================
\begin{abstract}
Sea $\Delta<0$ un discriminante fundamental, $K=\Q(\sqrt{\Delta})$ y
\[
x^2+y^2+z^2=R^2,\qquad z=c\ \Longrightarrow\ x^2+y^2=R^2-c^2.
\]
Esta identidad geométrica es el origen conceptual del trabajo: al cortar la esfera a altura $c$, el radio cuadrado del paralelo es
$R^2-c^2$. En el caso gaussiano ($\Delta=-4$), decidir si $R^2-c^2$ es norma equivale a decidir si se representa como suma de dos cuadrados;
en general, reemplazamos ``suma de dos cuadrados'' por pertenencia al conjunto de normas $N_{K/\Q}(\mathcal O_K)$.
\[
N_\Delta(R)=\#\{\,1\le c\le R-1:\ M_R(c)=R^2-c^2\in N_{K/\Q}(\mathcal O_K)\,\}.
\]
Mostramos que la dependencia de $N_\Delta(R)$ respecto de la factorización de $R$ en primos \emph{inertes} no es un efecto estadístico,
sino una oscilación local determinista originada en la estructura $p$-ádica de $M_R(c)=(R-c)(R+c)$.

El resultado central es local y es completamente explícito: para todo primo impar $p\ge 3$ calculamos en forma cerrada la probabilidad de que
$v_p(M_R(c))$ sea impar cuando $c$ es Haar-uniforme en $\Z_p$ y $m=v_p(R)$,
\[
\Prob\big(v_p(M_R(c))\ \mathrm{impar}\big)=\frac{2}{p^m(p+1)},
\qquad
\sigma_p^{(m)}=\Prob\big(v_p(M_R(c))\ \mathrm{par}\big)=1-\frac{2}{p^m(p+1)}.
\]
Además, cuando $2$ es inerte ($\Delta\equiv 5\!\!\pmod 8$), aparece un \emph{interruptor} racional $2$-ádico:
\[
\sigma_{2,\Delta}^{(m)}=1-\frac{1}{3\cdot 2^{m}},\qquad \sigma_{2,\Delta}^{(0)}=\frac{2}{3}.
\]
Estas identidades se explican por un principio estructural: fuera de la \emph{capa crítica} $v_p(c)=v_p(R)$ la paridad de $v_p(M_R(c))$
es forzosamente par.

Definimos un normalizador determinista $H_\Delta(R)$ como producto finito de cocientes $\sigma_{p,\Delta}^{(m)}/\sigma_{p,\Delta}^{(0)}$
sobre los primos inertes que dividen $R$ (incluyendo $p=2$ solo cuando es inerte y divide $R$) y consideramos el observable
\[
C_{\sigma,\Delta}(R)=\frac{N_\Delta(R)\log R}{H_\Delta(R)\,R}.
\]
Identificamos el baseline arquimediano canónico
\[
\Csbase(\Delta)=2e^{-\euler}L(1,\chi_\Delta)=2e^{-\euler}\mathrm{Res}_{s=1}\zeta_K(s),
\]
y en discriminantes de Heegner una forma cerrada en términos de $\sqrt{|\Delta|}$. Además, presentamos evidencia computacional
hasta $R\le 10^7$ de que $C_{\sigma,\Delta}(R)$ se estabiliza hacia un límite $\Csinf(\Delta)$ compatible con la descomposición
$\Csinf(\Delta)=\Csbase(\Delta)\,\mathfrak{S}_\Delta$, donde $\mathfrak{S}_\Delta$ es una serie singular local dominada por primos pequeños.
Finalmente formulamos un teorema global \emph{condicional}: bajo una hipótesis estándar de nivel de distribución para
$M_R(c)=R^2-c^2$ en progresiones aritméticas, se obtiene
\[
N_\Delta(R)=\Csinf(\Delta)\,H_\Delta(R)\,\frac{R}{\log R}+O\!\left(\frac{R}{(\log R)^{1+\delta}}\right)
\]
para algún $\delta>0$.
\end{abstract}
...
\toprule
{$\Delta$} & {$C_\infty^{(1)}$} & {$\mathrm{EE}_{\mathrm{WLS}}$} & {$\mathrm{EE}_{\mathrm{sist}}$} & {$|\Delta_{\mathrm{model}}|$} \\
\midrule
$-163$ & 0.156052 & 0.000063 & 0.000530 & 0.004047 \\
$-67$  & 0.241297 & 0.000107 & 0.000401 & 0.006613 \\
$-43$  & 0.303469 & 0.000140 & 0.001303 & 0.022085 \\
$-19$  & 0.473185 & 0.000208 & 0.001055 & 0.017593 \\
$-11$  & 0.650708 & 0.000281 & 0.001388 & 0.022101 \\
$-8$   & 0.692068 & 0.000070 & 0.001398 & 0.019602 \\
$-7$   & 0.431485 & 0.000186 & 0.000786 & 0.011712 \\
$-4$   & 0.735032 & 0.000064 & 0.001259 & 0.019624 \\
$-3$   & 0.566005 & 0.000065 & 0.001074 & 0.016527 \\
\bottomrule
\end{tabular}
\end{table}
...
La hipótesis $\mathcal D_\theta$ entrega la uniformidad en progresiones requerida para cerrar una criba de dimensión $1$ con ahorro en el error.
Los Teoremas~\ref{thm:local-odd} y \ref{thm:sigma2-inerte} fijan los factores locales inertes, y el Teorema~\ref{thm:arch}
fuerza el baseline arquimediano $2e^{-\euler}L(1,\chi_\Delta)$.
La convergencia absoluta del producto residual se obtiene por \Cref{cor:ssing-absconv}. Con esto, el término principal queda determinado y el error
hereda un ahorro logarítmico $O(R/(\log R)^{1+\delta})$.
\end{proof}
